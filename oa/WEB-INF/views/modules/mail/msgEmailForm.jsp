	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
		 ...
		 　　　 'uploader':'<%=basePath%>/servlet/fu?uid=${fns:getUser().id}',
				...
				'removeTimeout' : 0,
				//限制文件格式,不设置默认全部格式uploadify-button 
		            'fileTypeExts' : '',
		            //上传结束时调用的方法
		            ...
		            //alert("data:"+data);
		            	if (data==null||data.id==""||data.id==undefined||data.id==null) {
							...
							var li_="<li>";
							li_+="<a href=\"<%=basePath%>/servlet/fd?url=&fid="+data.id+"&fd=1&m=0\" url=\"\" target=\"_blank\">"+data.fileOname+"</a>&nbsp;&nbsp;<a href=\"javascript:\" onclick=\"fileDel('"+data.id+"',this)\">×</a>"
							li_+="</li>";
		...
		　　　});
			...
			$("#mail_send").click(function(){
				...
				//alert("fids:"+$("#fids").val(file_ids.join(",")));
				$("#inputForm").submit();
			});
			/*存草稿*/
			...
			/*部门人员，树型结构*/
			$.get("${ctx}/sys/office/treeData?type=3&extId=&isAll=true&module=&t="+new Date().getTime()
					+ new Date().getTime(), function(zNodes1){
				// 初始化树结构
				tree = $.fn.zTree.init($("#ztree1"), setting, zNodes1);
				
				// 默认展开一级节点
				var nodes1 = tree.getNodesByParam("level", 0);
				for(var i=0; i<nodes1.length; i++) {
					tree.expandNode(nodes1[i], true, false, false);
				}
				//异步加载子节点（加载用户）
				var nodesOne1 = tree.getNodesByParam("isParent", true);
				for(var j=0; j<nodesOne1.length; j++) {
					tree.reAsyncChildNodes(nodesOne1[j],"!refresh",true);
				}
			});
			/*选择用户*/
...
// 		}
		...
		**/
		...
		var zNodes1=[{id:1, pId:0, name:"[core] 基本功能 演示", open:false},
        ...
        <div class="row-fluid show-grid">
            ...
            <div class="row span10" style="width:100%;padding: 0px 5px;">
				<div class="mail-box-header">
                        <div class="input-append input-prepend pull-left" style="padding-top: 15px" id="send_operation">
				...
				<div class="mail-box">
                    <div class="mail-body">
                        <form:form id="inputForm" modelAttribute="msgEmail" style="padding-top: 40px" action="${ctx}/mail/msgEmail/save" method="post" class="form-horizontal">
                            <form:hidden path="id"/>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group">
                                    <form:input path="mailTitle" htmlEscape="false" maxlength="300" class="input-xlarge required" style="width:70.4%"/>
									<span class="help-inline" style="padding-left:8px;"><font color="red">*</font> </span>
                                </div>
                            </div>
					...
					<div class="container" style="position:absolute;height:375px;top:60px;right:30px;width:23%;border:1px solid #7f9db9">
							<ul class="tabs">  
								<li class="active"><a href="#tab1">常用</a></li>  
								<li><a href="#tab4">科室人员</a></li>
								<!--  <li><a href="#tab5">自定义分组</a></li> -->
							</ul>  
							<div class="tab_container">  
								...
								<div id="tab4" class="tab_content" style="display: none; ">  
								 ...
								 <%--   <div id="tab5" class="tab_content" style="display: none; ">  
										<!--  <input id="checked_" class="btn btn-primary btn-sm" type="button" value="选择"/>-->
										<input id="mail_common" class="btn btn-primary btn-sm" type="button" value="新建分组"/>
										<input id="mail_common" class="btn btn-primary btn-sm" type="button" value="删除分组"/>
										<ul  class="unstyled" id="mail121" style="padding:5px 0 0 10px;">
										   <c:forEach items="${mzdylist}" var="mz">
												<li>
													<input type="checkbox" name="link_user" mcid="${mz.id}" uid="${mc.linkUser.id}" uname="${mc.linkUser.name}" oname="${mc.linkUser.officeName}" class="link_user" value="">
													<a href="" url="" target="_blank">${mc.linkUser.name}</a>&nbsp;&nbsp;
													<a href="" url="" target="_blank">${mc.linkUser.officeName}</a>&nbsp;&nbsp;
													<a href="javascript:" onclick="common_remove('${mc.id}',this)">×</a>
												</li>
									...
									<!--  <div id="ztree1" class="ztree1" style="width:100%;overflow-y:auto;height:330px;"></div>-->
								</div> --%>
							</div>  
						</div>  
					<!--</div>-->
            ...
            <div class="mail-box-footer">
                        <div class="input-append input-prepend pull-left" style="padding-top: 15px" id="send_operation">
							...
							<a href="javascript:void(0);"><button id="btn_draft" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="bottom" title="保存草稿">存稿</button></a>&nbsp;&nbsp;
							<a href="javascript:void(0);"><button id="btn_cancel" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="bottom" title="放弃">放弃</button></a>&nbsp;&nbsp; -->
							<input id="mail_send1" class="btn btn-primary btn-sm" type="button" value="发送"/>
							<input id="btn_draft1" class="btn btn-primary btn-sm" type="button" value="存稿"/>
							<input id="btn_cancel1" class="btn btn-primary btn-sm" type="button" value="放弃"/>
                        </div>
