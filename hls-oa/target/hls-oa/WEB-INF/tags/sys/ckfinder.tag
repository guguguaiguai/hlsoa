...
<%@ attribute name="viewType" type="java.lang.String" required="false" description="文件显示方式：list-列表、thumbnails-缩略图"%>
...
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		var url = "${ctxStatic}/ckfinder/ckfinder.html?viewType=${viewType}&type=${ctype}&start=${ctype}:${uploadPath}/"+year+"/"+month+
