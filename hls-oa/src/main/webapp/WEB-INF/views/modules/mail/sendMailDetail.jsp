		...
		$(document).ready(function() {
			...
			if(files.indexOf("|")>=0){
				$.each(files.split("|"),function(i,d){
					if(d!=""){
						...
						fc+="<div class=\"file\">";
						fc+="<a href=\"../../../servlet/fd?url="+decodeURIComponent(d)+"&fid=&fd=${msgEmail.fdCount}&m=${msgEmail.id}\" style=\"line-height:30px;\">";
			...
			$("#btn_del").click(function(){
				...
				top.$.jBox.confirm("确定删除吗？", "提示", function(v,h,f){
					if (v == 'ok'){
						$.getJSON("${ctx}/mail/msgEmail/rmdel",{mids:id_},function(data){
							if(data.success){
								top.$.jBox.success('所选邮件删除成功！', '提示', { closed: function () { window.location.href="${ctx}/mail/msgEmail"; } });
							}else{
